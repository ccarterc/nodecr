  ...
  Tesseract.prototype.preprocessors = {
    convert: ConvertPreprocessor = function(inputFile, callback, preprocessOptions) {
      ...
      tmp.tmpName({
        ...
        if(preprocessOptions && preprocessOptions.length > 0){
          numOptions = preprocessOptions.length;

          for(i = 0; i < numOptions; i += 1){
            for(var propt in preprocessOptions[i]){
              if(preprocessOptions[i].hasOwnProperty(propt)){
                command.push(propt);
                command.push(preprocessOptions[i][propt]);
              }
            }
          }
        }
